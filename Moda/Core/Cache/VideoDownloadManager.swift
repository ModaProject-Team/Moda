        ...
        for attempt in 0..<maxRetryCount {
            do {
                ...
                if attempt < maxRetryCount - 1 {
                    print("[VideoDownloadManager] Download failed (attempt \(attempt + 1)/\(maxRetryCount)), retrying...")
                    try await Task.sleep(nanoseconds: UInt64(retryDelay * 1_000_000_000))
                }
            }
        }

        print("[VideoDownloadManager] Download failed after \(maxRetryCount) attempts")
        throw lastError ?? VideoCacheError.downloadFailed
    }
